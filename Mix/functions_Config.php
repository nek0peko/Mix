...
function themeConfig($form)
{
    ...
    // Page Title Suffix
    $HeaderDescription = new Text('HeaderDescription', NULL, _t('Create Your World!'), _t('首页标题后缀'), _t('你的博客标题栏博客名称后面的副标题，如果为空，则不显示副标题</br></br>'));
    ...
    // Blog Description
    $HeaderMore = new Text('HeaderMore', NULL, _t('Modify your name and photo and get started.'), _t('博主的介绍'), _t('这里填写的是文字'));
...
//    $form->addInput($BackColor);
    ...
    $form->addItem(new Typecho_Widget_Helper_Layout("/div"));
    $form->addItem(new Typecho_Widget_Helper_Layout("/div"));
    $form->addItem(new Title('图标设置', '图标对应的链接'));

    // BiliBili图标
    $HeaderBiliBili = new Text('HeaderBiliBili', NULL, _t('#'), _t('BiliBili'), _t('这里填写的是URL'));
    $form->addInput($HeaderBiliBili);

    // GitHub图标
    $HeaderGitHub = new Text('HeaderGitHub', NULL, _t('https://github.com/wibus-wee'), _t('GitHub'), _t('这里填写的是URL'));
    $form->addInput($HeaderGitHub);

    // Pixiv图标
    $HeaderPixiv = new Text('HeaderPixiv', NULL, _t('#'), _t('Pixiv'), _t('这里填写的是URL'));
    $form->addInput($HeaderPixiv);

//    // QQ图标
//    $HeaderQQ = new Text('HeaderQQ', NULL, _t('#'), _t('QQ图标链接'), _t('这里填写的是URL'));
...
//    $form->addInput($HeaderFourHTML);
    ...
    // 顶部导航自定义2.0
    $headnavItems = new Textarea('headnavItems', NULL, NULL, _t('顶部导航配置'), '<span style="color:red;">如果不明白此项，请清空该项，可不填写</span>');
...
//    $form->addInput($RSS_Site);
    ...
    // 自定义CSS
    $CSS = new Textarea('CSS', NULL, _t(''), _t('自定义CSS'), _t('这里填写的是css代码，来进行自定义样式，会自动输出到<\/head>标签之前'));
    $form->addInput($CSS);

    // 自定义JavaScript
    $JavaScript = new Textarea('JavaScript', NULL, _t(''), _t('自定义JavaScript'), _t('这里填写的是JavaScript代码，会自动输出到<\/body>标签之前'));
    $form->addInput($JavaScript);

    // 头部自定义输出HTML代码
    $HeaderHTML = new Textarea('HeaderHTML', NULL, _t(''), _t('自定义输出head头部的HTML代码'), _t('这里填写的是html代码，会输入到<\/head>之前</br>你可以填写网站统计代码等其他信息。</br>网站统计代码推荐google 统计和百度统计，不推荐cnzz（会导致样式错误，如果你会修改样式的话，请忽略该行）'));
    $form->addInput($HeaderHTML);

    // 底部自定义输出HTML代码
    $FooterHTML = new Textarea('FooterHTML', NULL, _t(''), _t('自定义输出body尾部的HTML代码'), _t('这里填写的是html代码，这里填写的是html代码，会输入到<\/body>之前'));
    ...
    // 博客底部右侧信息
    $RightHTML = new Textarea('RightHTML', NULL, _t(''), _t('博客底部右侧信息'), _t('这里面填写的是HTML代码，位置是博客底部的右边。'));
